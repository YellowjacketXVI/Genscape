...
import { LOCAL_MEDIA } from '@/utils/localMedia';

type ContentBrowserProps = {
  visible: boolean;
  onClose: () => void;
  onSelect: (media: { id: string; preview: any }) => void;
  widgetType: string;
};

// Use local development media
const MOCK_MEDIA = LOCAL_MEDIA.map((item) => ({
  id: item.id,
  type: item.type,
  title: item.title,
  ...
  const renderMediaItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => onSelect({ id: item.id, preview: item.thumbnail })}
      >
